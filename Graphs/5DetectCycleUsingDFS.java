...
public class Main {
    ...
    public static boolean detect_helper(ArrayList<ArrayList<Integer>> graph, int n, boolean[] vis, int node,
            int parent) {
        vis[node] = true;
        for (Integer x : graph.get(node)) {
            if (vis[x] == false) {
                boolean ans = detect_helper(graph, n, vis, x, node);
                return ans;
            } else if (x != parent) {
                return true;
            }
    ...
    public static boolean detectCycle(ArrayList<ArrayList<Integer>> graph, int n) {
        ...
        for (int i = 0; i < n; i++) {
            if (vis[i] == false) {
                if (detect_helper(graph, n, vis, i, -1)) {
                    return true;
                }
            }
        }
